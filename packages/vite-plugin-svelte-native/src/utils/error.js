...
export function enhanceCompileError(err, originalCode, preprocessors) {
	...
	normalizeNativeCompileError(err, originalCode);
...
/**
 * @param {import('svelte/compiler').Warning & Error & {frame?: string; code?: string; start?: { line: number; column: number }; end?: { line: number; column: number }}} err
 * @param {string} originalCode
 */
function normalizeNativeCompileError(err, originalCode) {
	if (err.name === 'SvelteNativeError') {
		err.name = 'CompileError';
	}

	if (err.code && !String(err.message).includes(`https://svelte.dev/e/${err.code}`)) {
		err.message += `\nhttps://svelte.dev/e/${err.code}`;
	}

	if (!err.frame && err.start) {
		err.frame = buildFrame(
			originalCode,
			err.start.line,
			err.end?.line === err.start.line ? err.end.column : err.start.column
		);
	}
}
 ...
 * @param {number} column
 */
function buildFrame(code, line, column) {
