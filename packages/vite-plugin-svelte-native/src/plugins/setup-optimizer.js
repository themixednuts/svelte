...
import { preprocessWithNative, transformWithNative } from '../utils/native.js';
...
async function compileSvelte(options, { filename, code }, statsCollection) {
	...
	if (options.preprocess) {
		try {
			preprocessed = await preprocessWithNative(code, options.preprocess, { filename });
		} catch (e) {
	...
	const compiled = await transformWithNative({
		id: filename,
		code: finalCode,
		ssr: false,
		...
		compilerOptions: finalCompileOptions
	});
	if (endStat) {
		endStat();
	}
