...
export default function __COMPONENT__($$anchor) {
	...
	var $$promises = $.run([async () => __BLOCKING_NAME__ = await $.async_derived(() => __FOO_NAME__)]);
	var fragment = root();
	...
	$.async(node, [$$promises[0]], void 0, (node) => {
		...
		$.if(node, ($$render) => {
			if (__FOO_NAME__) $$render(consequent); else if (bar) $$render(consequent_1, 1); else $$render(alternate, -1);
		});
	});

	var node_1 = $.sibling(node, 2);

	$.async(node_1, [$$promises[0]], [() => __FOO_NAME__], (node_1, $$condition) => {
	...
	$.async(node_3, [$$promises[0]], [async () => (await $.save(__FOO_NAME__))() > 10], (node_3, $$condition) => {
		...
		var alternate_4 = ($$anchor) => {
			...
			$.async(node_4, [$$promises[0]], [async () => (await $.save(__FOO_NAME__))() > 5], (node_4, $$condition) => {
...

	{
		...
		var d = $.derived(() => __COMPLEX_FN__() * complex2 > 100);
	...
	$.async(node_6, [$$promises[0]], void 0, (node_6) => {
		...
		$.if(node_6, ($$render) => {
			if ($.get(__BLOCKING_NAME__) > 10) $$render(consequent_11); else if ($.get(__BLOCKING_NAME__) > 5) $$render(consequent_12, 1); else $$render(alternate_6, -1);
		});
	});
