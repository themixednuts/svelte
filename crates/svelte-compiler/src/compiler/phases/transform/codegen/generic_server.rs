...
pub(crate) fn compile_generic_server_markup_js(
    ...
    compile_generic_markup_js::<ServerRenderBackend>(source, target, root, filename)
}
...
mod tests {
    ...
    fn generic_server_codegen_supports_expression_if_each_and_components() {
        ...
        let output = compile_generic_server_markup_js(source, GenerateTarget::Server, &root, None)
            .expect("generic server codegen");
