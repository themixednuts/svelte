...
use crate::api::modern::{RawField, estree_node_field};
...
impl<'src> SvelteOxcParser<'src> {
    ...
    pub(crate) fn parse_program_for_compile(&self) -> Option<EstreeNode> {
        ...
        loc_fields.insert(
            "start".to_string(),
            EstreeValue::Object(crate::api::position_raw_node(
        ...
        if program_body_is_empty(program) {
            let trailing =
                crate::api::parse_leading_comment_nodes(self.source, self.offsets.global_start);
    ...
    fn parse_and_normalize_raw_node(&self, estree_json: &str) -> Option<ParsedRawNode> {
        ...
        crate::api::normalize_estree_node(
            &mut node,
            self.source,
        ...
        let comments = crate::api::parse_all_comment_nodes(self.source, self.offsets.global_start);
        if !comments.is_empty() {
            crate::api::attach_estree_comments_to_tree(
                &mut node,
                self.source,
                self.offsets.global_start,
                &comments,
