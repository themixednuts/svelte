...
use super::{
    ...
    legacy_expression_from_raw_node, parse_identifier_name, parse_modern_attributes,
    ...
    for child in root.named_children(&mut cursor) {
        ...
        match child.kind() {
            ...
            kind if is_typed_block_kind(kind) => {
                pending_script_comment = None;
                if let Some(block_node) = parse_modern_block(source, child) {
            ...
            "element" => {
                    ...
                    parse_modern_collapsed_comment_tag_sequence(source, child)
                {
                    ...
                    previous_child_end = Some(child.end_byte());
                    continue;
                }
                ...
                fragment_nodes.push(parse_modern_element_node(
                    source, child, false, false, loose,
                ));
            }
            "ERROR" => {
                pending_script_comment = None;
...
fn collect_parse_errors(source: &str, root: TsNode<'_>) -> Vec<ParseError> {
    fn walk(
        ...
        if is_typed_block_kind(node.kind()) {
        ...
        } else if node.kind() == "ERROR" && !parent_kind.is_some_and(|k| is_typed_block_kind(k)) {
            ...
            if let Some(error) = parse_error_from_non_error_node(source, node) {
                errors.push(error);
            }
        }
...
fn collect_block_parse_errors(source: &str, block: TsNode<'_>, errors: &mut Vec<ParseError>) {
    ...
    if matches!(block_kind, BlockKind::Await) {
        if let Some(pending) = block.child_by_field_name("pending") {
...
fn missing_brace_in_block_start(block: TsNode<'_>) -> Option<usize> {
    ...
    for (i, child) in all.iter().enumerate() {
        if child.kind() == "ERROR" && child.is_named() {
            if all[i + 1..]
                .iter()
...
fn parse_error_from_error_node(source: &str, error: TsNode<'_>) -> Option<ParseError> {
    ...
    // Check for typed blocks inside ERROR (unclosed block recovery)
    {
        ...
        {
            if !has_named_descendant(typed_block, "block_end") {
...
fn parse_error_from_non_error_node(source: &str, node: TsNode<'_>) -> Option<ParseError> {
        ...
        && let Some(start_tag) = find_direct_named_child(node, "start_tag")
    {
        ...
        if find_direct_named_child(node, "end_tag").is_none()
            ...
            && node.end_byte() == source.len()
        {
            if let Some(tag_name) = find_direct_named_child(start_tag, "tag_name") {
...
pub(crate) fn modern_node_start(node: &Node) -> usize {
    node.start()
}

pub(crate) fn modern_node_end(node: &Node) -> usize {
    node.end()
}
...
pub(super) fn parse_modern_script(
    ...
    let content = crate::compiler::phases::parse::parse_modern_program_content_with_offsets(
        content_source,
        content_start,
    ...
    if let Some(comment) = leading_comment {
        attach_program_leading_comment(&mut content, comment);
    }
...
fn parse_modern_options_fragment(source: &str, element: TsNode<'_>) -> Fragment {
    ...
    for child in element.named_children(&mut cursor) {
        match child.kind() {
            ...
            kind if is_typed_block_kind(kind) => {
                if let Some(block) = parse_modern_block(source, child) {
                    nodes.push(block);
                }
            }
            "element" => nodes.push(parse_modern_element_node(
                source, child, false, false, false,
            )),
            "ERROR" => {
                let mut recovered = recover_modern_error_nodes(source, child, false);
...
fn attach_program_leading_comment(program: &mut EstreeNode, comment: &str) {
    if comment.is_empty() {
        return;
    }

    let mut comment_fields = BTreeMap::new();
    comment_fields.insert("type".to_string(), EstreeValue::String(Arc::from("Line")));
    comment_fields.insert("value".to_string(), EstreeValue::String(Arc::from(comment)));

    match estree_node_field_mut(program, RawField::LeadingComments) {
        Some(EstreeValue::Array(existing)) => {
            let mut next = Vec::with_capacity(existing.len() + 1);
            next.push(EstreeValue::Object(EstreeNode {
                fields: comment_fields,
            }));
            next.extend(existing.iter().cloned());
            *existing = next.into_boxed_slice();
        }
        _ => {
            program.fields.insert(
                "leadingComments".to_string(),
                EstreeValue::Array(
                    vec![EstreeValue::Object(EstreeNode {
                        fields: comment_fields,
                    })]
                    .into_boxed_slice(),
                ),
...
fn empty_program_raw(
    ...
    program.insert(
        "type".to_string(),
        EstreeValue::String(Arc::from("Program")),
    );
    program.insert("start".to_string(), EstreeValue::UInt(start as u64));
    program.insert("end".to_string(), EstreeValue::UInt(end as u64));
    program.insert("body".to_string(), EstreeValue::Array(Box::new([])));
    program.insert(
        "sourceType".to_string(),
        EstreeValue::String(Arc::from("module")),
    );

    let mut loc = BTreeMap::new();
    loc.insert(
        "start".to_string(),
        EstreeValue::Object(position_raw_node(start_line, start_column)),
    );
    loc.insert(
        "end".to_string(),
        EstreeValue::Object(position_raw_node(end_line, end_column)),
    );
    program.insert(
        "loc".to_string(),
        EstreeValue::Object(EstreeNode { fields: loc }),
    );

    EstreeNode { fields: program }
}
...
fn parse_modern_block(source: &str, block: TsNode<'_>) -> Option<Node> {
    match block.kind() {
...
fn parse_const_tag_declaration(source: &str, tag: TsNode<'_>) -> Option<Expression> {
    ...
    let program = crate::compiler::phases::parse::parse_modern_program_content_with_offsets(
        declaration_source,
        tag.start_byte() + 2,
    ...
    let declarations = estree_node_field_array(&program, RawField::Body)?;
    let [EstreeValue::Object(declaration)] = declarations else {
        return None;
    };

    Some(Expression(declaration.clone(), Default::default()))
}
...
pub(crate) fn split_debug_tag_arguments(expression: Expression) -> Box<[Expression]> {
    if estree_node_type(&expression.0) != Some("SequenceExpression") {
        return vec![expression].into_boxed_slice();
    }

    let Some(expressions) = estree_node_field_array(&expression.0, RawField::Expressions) else {
        return vec![expression].into_boxed_slice();
    };
    ...
    expressions
        ...
        .filter_map(|value| match value {
            EstreeValue::Object(argument) => Some(Expression(argument.clone(), Default::default())),
            _ => None,
        })
        .collect::<Vec<_>>()
        .into_boxed_slice()
...
fn modern_identifier_from_expression(expression: Expression) -> Option<Identifier> {
    ...
    if estree_node_type(&raw) != Some("Identifier") {
        return None;
    }
    let name = estree_node_field_str(&raw, RawField::Name)?;
...
pub(crate) fn find_matching_brace_close(
    source: &str,
    open_index: usize,
    limit: usize,
) -> Option<usize> {
...
fn parse_modern_binding_field_with_error(
    ...
    if let Some(mut expression) = parse_pattern_with_oxc(trimmed, abs, line, column) {
        if with_character {
            set_expression_character(&mut expression);
        }
    ...
    if let Some(expression) = parse_modern_expression_from_text(trimmed, abs, line, column)
        && let Some((start, message)) = invalid_binding_expression_error(&expression.0)
    {
        return (
            None,
            Some(ParseError {
...
fn invalid_binding_expression_error(expression: &EstreeNode) -> Option<(usize, Arc<str>)> {
    match estree_node_type(expression) {
        Some("ObjectExpression") => invalid_object_binding_expression_error(expression),
        Some("ArrayExpression") => invalid_array_binding_expression_error(expression),
        _ => None,
    }
}

fn invalid_object_binding_expression_error(expression: &EstreeNode) -> Option<(usize, Arc<str>)> {
    let properties = estree_node_field_array(expression, RawField::Properties)?;
    for (index, property) in properties.iter().enumerate() {
        let EstreeValue::Object(property) = property else {
            continue;
        };
        if estree_node_type(property) == Some("SpreadElement") && index + 1 < properties.len() {
            let start = estree_node_bounds(property)
                .map(|(_, end)| end)
                .unwrap_or(0);
            return Some((
                start,
        ...
        if estree_node_type(property) == Some("Property")
            ...
            && let Some(error) = invalid_binding_expression_error(value)
        {
            return Some(error);
        }
    }
...
fn invalid_array_binding_expression_error(expression: &EstreeNode) -> Option<(usize, Arc<str>)> {
    let elements = estree_node_field_array(expression, RawField::Elements)?;
    for (index, element) in elements.iter().enumerate() {
        let EstreeValue::Object(element) = element else {
            continue;
        };
        if estree_node_type(element) == Some("SpreadElement") && index + 1 < elements.len() {
            let start = estree_node_bounds(element).map(|(_, end)| end).unwrap_or(0);
            return Some((
                start,
                Arc::from("Comma is not permitted after the rest element"),
            ));
...
fn parse_pattern_error_from_text(
    ...
    crate::compiler::phases::parse::parse_modern_expression_error_detail_with_oxc(
        &wrapped,
...
pub(crate) fn parse_snippet_params(
    ...
    let params = named_children_vec(params_node)
        .into_iter()
...
pub(crate) fn parse_pattern_with_oxc(
    ...
    let wrapped = format!("({text})=>{{}}", text = text);
    let global_start = abs_start.saturating_sub(1);
    let base_column = column.saturating_sub(1);
    let expression = crate::compiler::phases::parse::parse_modern_expression_with_oxc(
        &wrapped,
        global_start,
        line,
        base_column,
    )?;
    let Expression(raw, _) = expression;

    let params = estree_node_field(&raw, RawField::Params)?;
    let EstreeValue::Array(params) = params else {
        return None;
    };
    ...
    match first {
        EstreeValue::Object(node) => {
            ...
            if should_bump_pattern_columns(text) {
                bump_raw_node_columns(&mut node, 1);
            }
            normalize_pattern_template_elements(&mut node);
            simplify_pattern_parenthesized(&mut node);
            Some(Expression(node, Default::default()))
        }
        _ => None,
    }
}

fn should_bump_pattern_columns(text: &str) -> bool {
    let trimmed = text.trim();
    trimmed.starts_with('[')
        || trimmed.contains('`')
        || trimmed.contains("'{")
...
fn bump_raw_node_columns(node: &mut EstreeNode, amount: usize) {
    for (key, value) in &mut node.fields {
        if key == "column" {
            match value {
                EstreeValue::UInt(v) => {
                ...
                EstreeValue::Int(v) => {
                    *v = v.saturating_add(amount as i64);
                }
                _ => {}
            }
            continue;
        ...
        bump_raw_value_columns(value, amount);
    }
}

fn bump_raw_value_columns(value: &mut EstreeValue, amount: usize) {
    match value {
        EstreeValue::Object(node) => bump_raw_node_columns(node, amount),
        EstreeValue::Array(items) => {
            for item in items.iter_mut() {
                bump_raw_value_columns(item, amount);
            }
        }
...
pub(crate) fn normalize_pattern_template_elements(node: &mut EstreeNode) {
    if matches!(
        estree_node_field(node, RawField::Type),
        Some(EstreeValue::String(kind)) if kind.as_ref() == "TemplateElement"
    ) {
        let is_tail = matches!(
            estree_node_field(node, RawField::Tail),
            Some(EstreeValue::Bool(true))
        );
        let end_delta = if is_tail { -1 } else { -2 };
        adjust_raw_start(node, 1);
        adjust_raw_end(node, end_delta);
        adjust_raw_loc_start_column(node, 1);
        adjust_raw_loc_end_column(node, end_delta);
    }

    for value in node.fields.values_mut() {
        normalize_pattern_template_value(value);
    }
}

fn normalize_pattern_template_value(value: &mut EstreeValue) {
    match value {
        EstreeValue::Object(node) => normalize_pattern_template_elements(node),
        EstreeValue::Array(items) => {
            for item in items.iter_mut() {
                normalize_pattern_template_value(item);
            }
        }
        EstreeValue::String(_)
        | EstreeValue::Int(_)
...
fn adjust_raw_start(node: &mut EstreeNode, delta: isize) {
    if let Some(value) = estree_node_field_mut(node, RawField::Start) {
        adjust_raw_number(value, delta);
    }
}

fn adjust_raw_end(node: &mut EstreeNode, delta: isize) {
    if let Some(value) = estree_node_field_mut(node, RawField::End) {
        adjust_raw_number(value, delta);
    }
}

fn adjust_raw_loc_start_column(node: &mut EstreeNode, delta: isize) {
    let Some(EstreeValue::Object(loc)) = estree_node_field_mut(node, RawField::Loc) else {
        return;
    };
    let Some(EstreeValue::Object(start)) = estree_node_field_mut(loc, RawField::Start) else {
        return;
    };
    if let Some(value) = estree_node_field_mut(start, RawField::Column) {
        adjust_raw_number(value, delta);
    }
}

fn adjust_raw_loc_end_column(node: &mut EstreeNode, delta: isize) {
    let Some(EstreeValue::Object(loc)) = estree_node_field_mut(node, RawField::Loc) else {
        return;
    };
    let Some(EstreeValue::Object(end)) = estree_node_field_mut(loc, RawField::End) else {
        return;
    };
    if let Some(value) = estree_node_field_mut(end, RawField::Column) {
        adjust_raw_number(value, delta);
    }
}

fn adjust_raw_number(value: &mut EstreeValue, delta: isize) {
    match value {
        EstreeValue::UInt(v) => {
            if delta >= 0 {
        ...
        EstreeValue::Int(v) => {
            if delta >= 0 {
                *v = v.saturating_add(delta as i64);
            } else {
                *v = v.saturating_sub((-delta) as i64);
            }
        }
        _ => {}
    }
...
fn simplify_pattern_parenthesized(node: &mut EstreeNode) {
    for value in node.fields.values_mut() {
        simplify_pattern_parenthesized_value(value);
    }
}

fn simplify_pattern_parenthesized_value(value: &mut EstreeValue) {
    match value {
        EstreeValue::Object(node) => {
            for child in node.fields.values_mut() {
                simplify_pattern_parenthesized_value(child);
            }

            let is_parenthesized = matches!(
                estree_node_field(node, RawField::Type),
                Some(EstreeValue::String(kind)) if kind.as_ref() == "ParenthesizedExpression"
            );

            if is_parenthesized
                && let Some(EstreeValue::Object(inner)) =
                    estree_node_field(node, RawField::Expression)
            {
                *value = EstreeValue::Object(inner.clone());
            }
        }
        EstreeValue::Array(items) => {
            for item in items.iter_mut() {
                simplify_pattern_parenthesized_value(item);
            }
        }
        EstreeValue::String(_)
        | EstreeValue::Int(_)
...
pub(crate) fn line_column_at_offset(source: &str, offset: usize) -> (usize, usize) {
    let mut line = 1usize;
    let mut column = 0usize;
    let limit = offset.min(source.len());
    for ch in source[..limit].chars() {
        if ch == '\n' {
            line += 1;
            column = 0;
        } else {
...
fn set_expression_character(expression: &mut Expression) {
    let node = &mut expression.0;
    ...
    if let Some(EstreeValue::Object(start_loc)) = estree_node_field_mut(loc, RawField::Start) {
        start_loc
            .fields
            .insert("character".to_string(), EstreeValue::UInt(start as u64));
    }
    if let Some(EstreeValue::Object(end_loc)) = estree_node_field_mut(loc, RawField::End) {
        end_loc
            .fields
            .insert("character".to_string(), EstreeValue::UInt(end as u64));
    }
}
...
fn parse_modern_element_node(
    ...
    let element =
        parse_modern_regular_element(source, node, in_shadowroot_template, in_svelte_head, loose);
...
fn modern_string_literal_expression(value: Arc<str>, start: usize, end: usize) -> Expression {
    ...
    fields.insert(
        "type".to_string(),
        EstreeValue::String(Arc::from("Literal")),
    );
    fields.insert("start".to_string(), EstreeValue::UInt(start as u64));
    fields.insert("end".to_string(), EstreeValue::UInt(end as u64));
    fields.insert("value".to_string(), EstreeValue::String(value.clone()));
    fields.insert(
        "raw".to_string(),
        EstreeValue::String(Arc::from(format!(
            "'{}'",
...
fn parse_modern_regular_element(
    ...
    let name_loc = if let Some(tag_name) = tag_name {
        LegacyNameLocation {
            start: source_location_from_point(tag_name.start_position(), tag_name.start_byte()),
            end: source_location_from_point(tag_name.end_position(), tag_name.end_byte()),
        }
    } else {
        LegacyNameLocation {
            start: source_location_from_point(node.start_position(), node.start_byte()),
            end: source_location_from_point(node.start_position(), node.start_byte()),
        }
    };
    ...
    while index < nodes.len() {
        ...
        match node.kind() {
            ...
            "start_tag" => {
                ...
                for lookahead in (index + 1)..nodes.len() {
                    if is_loose_start_tag_boundary(nodes[lookahead]) {
                        stop = lookahead;
                        break;
                    }
                }
            ...
            kind if is_typed_block_kind(kind) => {
                if let Some(block_node) = parse_modern_block(source, node) {
                    out.push(block_node);
                }
            }
            kind if is_typed_tag_kind(kind) => {
...
pub(crate) fn body_start_index(
    ...
    for name in field_names {
        if let Some(field_node) = block.child_by_field_name(name) {
            if let Some(idx) = children.iter().position(|c| c.id() == field_node.id()) {
...
pub(crate) fn parse_modern_expression_tag(source: &str, node: TsNode<'_>) -> Option<ExpressionTag> {
...
fn loose_tag_name_and_loc(
    ...
    if let Some((name, name_end)) = loose_tag_name_range(source, name_start, fallback_end) {
        let (end_line, end_column) = line_column_at_offset(source, name_end);
        return (
            name,
            LegacyNameLocation {
    ...
    (
        Arc::from(""),
        LegacyNameLocation {
            start: source_location_from_point(container.start_position(), container.start_byte()),
            end: source_location_from_point(container.start_position(), container.start_byte()),
        },
    )
}
...
fn parse_modern_expression_error(source: &str, node: TsNode<'_>) -> Option<(usize, Arc<str>)> {
    ...
    if node.kind() == "expression" && raw.len() >= 2 && raw.starts_with('{') && raw.ends_with('}') {
        return parse_modern_expression_error_from_text(
            &raw[1..raw.len().saturating_sub(1)],
            node.start_byte().saturating_add(1),
            node.start_position().row + 1,
            node.start_position().column + 1,
    ...
    parse_modern_expression_error_from_text(
        raw,
        node.start_byte(),
        node.start_position().row + 1,
        node.start_position().column,
...
pub(crate) fn modern_empty_identifier_expression(node: TsNode<'_>) -> Expression {
...
fn modern_empty_identifier_expression_span(start: usize, len: usize) -> Expression {
    ...
    fields.insert(
        "type".to_string(),
        EstreeValue::String(Arc::from("Identifier")),
    );
    fields.insert("start".to_string(), EstreeValue::UInt(start as u64));
    fields.insert("end".to_string(), EstreeValue::UInt(end as u64));
...
fn modern_identifier_expression_with_loc(
    ...
    fields.insert("start".to_string(), EstreeValue::UInt(start as u64));
    fields.insert("end".to_string(), EstreeValue::UInt(end as u64));
    fields.insert("name".to_string(), EstreeValue::String(name));

    let mut loc_fields = BTreeMap::new();
    let mut start_fields = BTreeMap::new();
    start_fields.insert("line".to_string(), EstreeValue::UInt(line as u64));
    start_fields.insert("column".to_string(), EstreeValue::UInt(column as u64));
    start_fields.insert("character".to_string(), EstreeValue::UInt(start as u64));
    let mut end_fields = BTreeMap::new();
    end_fields.insert("line".to_string(), EstreeValue::UInt(line as u64));
    end_fields.insert(
        "column".to_string(),
        EstreeValue::UInt((column + (end - start)) as u64),
    );
    end_fields.insert("character".to_string(), EstreeValue::UInt(end as u64));
    loc_fields.insert(
        "start".to_string(),
        EstreeValue::Object(EstreeNode {
            fields: start_fields,
        }),
    );
    loc_fields.insert(
        "end".to_string(),
        EstreeValue::Object(EstreeNode { fields: end_fields }),
    );
    fields.insert(
        "loc".to_string(),
...
pub(crate) fn parse_modern_expression_from_text(
    text: &str,
    start_byte: usize,
    line: usize,
    column: usize,
    ...
    let mut raw = crate::compiler::phases::parse::parse_modern_expression_with_oxc(
        trimmed, start, start_line, start_col,
    )?;
    let parens = unwrap_parenthesized_expression(&mut raw);
    raw.1.parens = if parens == 0 {
        leading_parens(trimmed, start, &raw.0)
    } else {
        parens
    };
...
fn parse_modern_expression_error_from_text(
    ...
    let start = start_byte + leading_ws;
    let (start_line, start_col) = offset_to_line_column(text, leading_ws, line, column);
    let message = crate::compiler::phases::parse::parse_modern_expression_error_with_oxc(
        trimmed, start, start_line, start_col,
    )?;
    Some((start, message))
}

fn unwrap_parenthesized_expression(expression: &mut Expression) -> u16 {
    let mut parens = 0u16;

    while estree_node_type(&expression.0) == Some("ParenthesizedExpression") {
        let Some(inner) = estree_node_field_object(&expression.0, RawField::Expression) else {
            break;
        };
        expression.0 = inner.clone();
...
fn leading_parens(text: &str, start: usize, expression: &EstreeNode) -> u16 {
    let Some(node_start) = estree_value_to_usize(estree_node_field(expression, RawField::Start))
    else {
        return 0;
    };
...
#[allow(dead_code)]
fn modern_loc_from_raw_node(node: &EstreeNode) -> Option<Loc> {
    let loc = estree_node_field_object(node, RawField::Loc)?;
    let start = estree_node_field_object(loc, RawField::Start)?;
    let end = estree_node_field_object(loc, RawField::End)?;

    Some(Loc {
        start: Position {
            line: estree_value_to_usize(estree_node_field(start, RawField::Line))?,
            column: estree_value_to_usize(estree_node_field(start, RawField::Column))?,
            character: estree_value_to_usize(estree_node_field(start, RawField::Character)),
        },
        end: Position {
            line: estree_value_to_usize(estree_node_field(end, RawField::Line))?,
            column: estree_value_to_usize(estree_node_field(end, RawField::Column))?,
            character: estree_value_to_usize(estree_node_field(end, RawField::Character)),
        },
    })
}

pub(crate) fn attach_leading_comments_to_expression(
    expression: &mut Expression,
    source: &str,
    global_start: usize,
) {
    let Expression(node, _) = expression;

    let comments = parse_leading_comment_nodes(source, global_start);
    ...
    node.fields.insert(
        "leadingComments".to_string(),
        EstreeValue::Array(comment_values),
    );
}

pub(crate) fn attach_trailing_comments_to_expression(
    expression: &mut Expression,
    source: &str,
    global_start: usize,
) {
    ...
    let comments = parse_trailing_comment_nodes(source, global_start, end);
    if comments.is_empty() {
        return;
    }
...
pub(crate) fn parse_leading_comment_nodes(source: &str, global_start: usize) -> Vec<EstreeNode> {
    let bytes = source.as_bytes();
    let mut i = 0usize;
    let mut comments = Vec::new();
    ...
    loop {
        ...
        if bytes[i] == b'/' && bytes[i + 1] == b'/' {
            ...
            let value_end = i;
            comments.push(comment_raw_node(
                "Line",
                source.get(value_start..value_end).unwrap_or_default(),
                global_start + start,
                global_start + value_end,
            ));
            continue;
        }
...
fn parse_trailing_comment_nodes(
    source: &str,
    global_start: usize,
    expression_end: usize,
) -> Vec<EstreeNode> {
    let mut i = expression_end.saturating_sub(global_start);
    let bytes = source.as_bytes();
    let mut comments = Vec::new();
...
pub(crate) fn parse_all_comment_nodes(source: &str, global_start: usize) -> Vec<EstreeNode> {
    let bytes = source.as_bytes();
    let mut i = 0usize;
    let mut out = Vec::new();
...
pub(crate) fn attach_estree_comments_to_tree(
    root: &mut EstreeNode,
    source_fragment: &str,
    global_start: usize,
    comments: &[EstreeNode],
) {
    attach_raw_comments_to_node(root, source_fragment, global_start, comments);
}

fn attach_raw_comments_to_node(
    node: &mut EstreeNode,
    source_fragment: &str,
    global_start: usize,
    comments: &[EstreeNode],
    ...
    let (container_start, container_end) =
        estree_node_bounds(node).unwrap_or((global_start, global_start + source_fragment.len()));

    for value in node.fields.values_mut() {
        match value {
            EstreeValue::Object(child) => {
                attach_raw_comments_to_node(child, source_fragment, global_start, comments);
            }
            EstreeValue::Array(items) => {
                attach_comments_to_raw_array(
                    items,
                    source_fragment,
                    global_start,
                    container_start,
                ...
                for item in items.iter_mut() {
                    if let EstreeValue::Object(child) = item {
                        attach_raw_comments_to_node(child, source_fragment, global_start, comments);
                    }
                }
            }
...
fn attach_comments_to_raw_array(
    items: &mut [EstreeValue],
    source_fragment: &str,
    global_start: usize,
    ...
    for (index, item) in items.iter().enumerate() {
        if let EstreeValue::Object(node) = item
            && let Some((start, end)) = estree_node_bounds(node)
        {
            entries.push((index, start, end));
        }
    }

    for (pos, (index, start, end)) in entries.iter().enumerate() {
        ...
        let leading = comments
            .iter()
            .filter_map(|comment| {
                let (comment_start, comment_end) = estree_node_bounds(comment)?;
                if comment_end > *start || comment_start < prev_end {
                    return None;
                }
                if comment_is_line(comment)
                    && source_segment(source_fragment, global_start, prev_end, comment_start)
                        .is_some_and(|text| !text.contains('\n'))
                {
                    return None;
                }
                let between = source_segment(source_fragment, global_start, comment_end, *start)?;
                if !is_js_comment_or_whitespace(between) {
                    return None;
                }
                Some((comment_start, comment_end, comment.clone()))
            })
            .collect::<Vec<_>>();
        ...
        let trailing = comments
            ...
            .filter_map(|comment| {
                let (comment_start, comment_end) = estree_node_bounds(comment)?;
                if comment_start < *end || comment_start >= next_start {
                    return None;
                }
                let between = source_segment(source_fragment, global_start, *end, comment_start)?;
                let allowed = if pos + 1 < entries.len() {
                    !between.contains('\n') && is_js_comment_whitespace_or_punct(between)
                } else {
                    is_js_comment_whitespace_or_punct(between)
                };
                if !allowed {
        ...
        if let EstreeValue::Object(node) = &mut items[*index] {
            if !leading.is_empty() {
                merge_raw_comment_field(
                    node,
                    "leadingComments",
                    leading
                        .iter()
                        .map(|(_, _, comment)| comment.clone())
                        .collect::<Vec<_>>(),
                );
            }
...
fn source_segment(
    ...
    let local_start = absolute_start.checked_sub(global_start)?;
    let local_end = absolute_end.checked_sub(global_start)?;
    source_fragment.get(local_start..local_end)
}

fn comment_is_line(comment: &EstreeNode) -> bool {
    matches!(
        estree_node_field(comment, RawField::Type),
        Some(EstreeValue::String(kind)) if kind.as_ref() == "Line"
    )
}

fn merge_raw_comment_field(node: &mut EstreeNode, key: &str, additional: Vec<EstreeNode>) {
    let mut merged = Vec::new();
    if let Some(EstreeValue::Array(existing)) = node.fields.get(key) {
        for value in existing.iter() {
            if let EstreeValue::Object(comment) = value {
    ...
    merged.retain(|comment| {
        let Some((start, end)) = estree_node_bounds(comment) else {
            return false;
        };
        let kind = comment
            .fields
            ...
            .and_then(|value| match value {
                EstreeValue::String(kind) => Some(kind.as_ref()),
                _ => None,
            })
            .unwrap_or("");
        seen.insert((start, end, kind.to_string()))
    });
    merged.sort_by_key(|comment| {
        estree_node_bounds(comment)
            .map(|(start, _)| start)
            .unwrap_or(usize::MAX)
    });
...
fn is_js_comment_or_whitespace(text: &str) -> bool {
    is_js_comment_whitespace_with_punct(text, false)
}

fn is_js_comment_whitespace_or_punct(text: &str) -> bool {
    is_js_comment_whitespace_with_punct(text, true)
}

fn is_js_comment_whitespace_with_punct(text: &str, allow_punct: bool) -> bool {
    let bytes = text.as_bytes();
    let mut i = 0usize;

    while i < bytes.len() {
        let b = bytes[i];
        if b.is_ascii_whitespace() || (allow_punct && (b == b',' || b == b';')) {
            i += 1;
            continue;
        }
...
fn comment_raw_node(kind: &str, value: &str, start: usize, end: usize) -> EstreeNode {
    let mut fields = BTreeMap::new();
    fields.insert("type".to_string(), EstreeValue::String(Arc::from(kind)));
    fields.insert("value".to_string(), EstreeValue::String(Arc::from(value)));
    fields.insert("start".to_string(), EstreeValue::UInt(start as u64));
    fields.insert("end".to_string(), EstreeValue::UInt(end as u64));
    EstreeNode { fields }
}

pub(crate) fn normalize_estree_node(
    node: &mut EstreeNode,
    source_fragment: &str,
    global_offset: usize,
    base_line: usize,
    ...
    node.fields.remove("range");

    let null_keys_to_drop = node
        .fields
        .iter()
        .filter_map(|(key, value)| {
            (matches!(value, EstreeValue::Null) && !matches!(key.as_str(), "id" | "init"))
                .then_some(key.clone())
        })
        .collect::<Vec<_>>();
    for key in null_keys_to_drop {
        node.fields.remove(&key);
    }

    if let Some(EstreeValue::Array(items)) = estree_node_field(node, RawField::Decorators)
        && items.is_empty()
    {
        node.fields.remove("decorators");
    }

    if matches!(
        estree_node_field(node, RawField::Optional),
        Some(EstreeValue::Bool(false))
    ) && matches!(estree_node_field(node, RawField::Type), Some(EstreeValue::String(kind)) if kind.as_ref() != "CallExpression" && kind.as_ref() != "MemberExpression")
    {
        node.fields.remove("optional");
    }

    if matches!(
        estree_node_field(node, RawField::Definite),
        Some(EstreeValue::Bool(false))
    ) {
        node.fields.remove("definite");
    }
    if matches!(
        estree_node_field(node, RawField::Declare),
        Some(EstreeValue::Bool(false))
    ) {
        node.fields.remove("declare");
    }
    ...
    for key in [
        ...
        if matches!(node.fields.get(key), Some(EstreeValue::Null)) {
            node.fields.remove(key);
        }
    }

    let rel_start = estree_value_to_usize(estree_node_field(node, RawField::Start));
    let rel_end = estree_value_to_usize(estree_node_field(node, RawField::End));
    if let (Some(rel_start), Some(rel_end)) = (rel_start, rel_end) {
        let abs_start = global_offset + rel_start;
        let abs_end = global_offset + rel_end;
        node.fields
            .insert("start".to_string(), EstreeValue::UInt(abs_start as u64));
        node.fields
            .insert("end".to_string(), EstreeValue::UInt(abs_end as u64));

        let start_pos = offset_to_line_column(source_fragment, rel_start, base_line, base_column);
        let end_pos = offset_to_line_column(source_fragment, rel_end, base_line, base_column);

        let mut loc_fields = BTreeMap::new();
        loc_fields.insert(
            "start".to_string(),
            EstreeValue::Object(position_raw_node(start_pos.0, start_pos.1)),
        );
        loc_fields.insert(
            "end".to_string(),
            EstreeValue::Object(position_raw_node(end_pos.0, end_pos.1)),
        );
        node.fields.insert(
            "loc".to_string(),
            EstreeValue::Object(EstreeNode { fields: loc_fields }),
...
fn normalize_raw_value(
    ...
    match value {
        EstreeValue::Object(node) => {
            normalize_estree_node(node, source_fragment, global_offset, base_line, base_column)
        }
        EstreeValue::Array(items) => {
            for item in items.iter_mut() {
                normalize_raw_value(item, source_fragment, global_offset, base_line, base_column);
            }
        }
        EstreeValue::String(_)
        | EstreeValue::Int(_)
...
pub(crate) fn estree_value_to_usize(value: Option<&EstreeValue>) -> Option<usize> {
    match value {
        Some(EstreeValue::Int(v)) => usize::try_from(*v).ok(),
        Some(EstreeValue::UInt(v)) => usize::try_from(*v).ok(),
        Some(EstreeValue::Number(v)) => v
            .as_u64()
            .and_then(|value| usize::try_from(value).ok())
            .or_else(|| v.as_i64().and_then(|value| usize::try_from(value).ok())),
        _ => None,
    }
}
...
pub(crate) fn estree_node_type(node: &EstreeNode) -> Option<&str> {
    estree_node_field_str(node, RawField::Type)
}

pub(crate) fn estree_node_field(node: &EstreeNode, key: RawField) -> Option<&EstreeValue> {
    node.fields.get(key.as_str())
}

pub(crate) fn estree_node_field_mut(
    node: &mut EstreeNode,
    key: RawField,
) -> Option<&mut EstreeValue> {
    node.fields.get_mut(key.as_str())
}

pub(crate) fn estree_node_has_field(node: &EstreeNode, key: RawField) -> bool {
    node.fields.contains_key(key.as_str())
}

pub(crate) fn estree_node_field_object(node: &EstreeNode, key: RawField) -> Option<&EstreeNode> {
...
pub(crate) fn estree_node_field_array(node: &EstreeNode, key: RawField) -> Option<&[EstreeValue]> {
    match estree_node_field(node, key) {
...
pub(crate) fn estree_node_field_str(node: &EstreeNode, key: RawField) -> Option<&str> {
    match estree_node_field(node, key) {
...
pub(crate) fn walk_estree_node(node: &EstreeNode, visitor: &mut impl FnMut(&EstreeNode)) {
    visitor(node);
    for value in node.fields.values() {
        walk_raw_value(value, visitor);
    }
}

pub(crate) fn walk_raw_value(value: &EstreeValue, visitor: &mut impl FnMut(&EstreeNode)) {
    match value {
        EstreeValue::Object(node) => walk_estree_node(node, visitor),
...
pub(crate) fn modern_node_span(node: &Node) -> (usize, usize) {
    (node.start(), node.end())
}
...
pub(crate) fn expression_identifier_name(expression: &Expression) -> Option<Arc<str>> {
    ...
    estree_node_field_str(node, RawField::Name).map(Arc::from)
}
...
fn estree_node_bounds(node: &EstreeNode) -> Option<(usize, usize)> {
...
pub(crate) fn position_raw_node(line: usize, column: usize) -> EstreeNode {
    let mut fields = BTreeMap::new();
    fields.insert("line".to_string(), EstreeValue::UInt(line as u64));
    fields.insert("column".to_string(), EstreeValue::UInt(column as u64));
    EstreeNode { fields }
}
...
pub(crate) fn legacy_expression_from_modern_expression(
    expression: Expression,
    include_character: bool,
) -> Option<LegacyExpression> {
    legacy_expression_from_raw_node(expression.0, include_character)
}
