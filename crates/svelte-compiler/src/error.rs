...
impl CompilerDiagnosticKind {
    ...
    pub fn to_compile_error(self, source: &str, start: usize, end: usize) -> CompileError {
        ...
        let start_utf16 = remap_offset(start, &utf16_map);
        let end_utf16 = remap_offset(end, &utf16_map);
...
fn remap_offset(offset: usize, map: &[usize]) -> usize {
    let bounded = offset.min(map.len().saturating_sub(1));
    map[bounded]
}
