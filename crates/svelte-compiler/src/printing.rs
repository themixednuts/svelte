use crate::api::modern::{RawField, estree_node_field, expression_identifier_name};
...
use crate::ast::modern::{
    ...
    CssRule, CssSelectorList, CssSimpleSelector, EachBlock, EstreeNode, EstreeValue, Expression,
    ExpressionTag, Fragment, IfBlock, Node, Options, Script,
};
...
fn render_program_body(
    ...
    if options.get_leading_comments.is_some() || options.get_trailing_comments.is_some() {
        return render_program_body_with_comment_hooks(program, depth, options);
    }

    render_program_body_from_source(source, program, depth)
}

fn render_program_body_from_source(source: &str, program: &EstreeNode, depth: usize) -> String {
    let Some(EstreeValue::Array(body)) = estree_node_field(program, RawField::Body) else {
        return String::new();
    };
    ...
    for value in body.iter() {
        ...
        let Some((start, end)) = estree_node_bounds(node) else {
            continue;
        };
        let snippet = source_slice(source, start, end).unwrap_or_default();
        if snippet.trim().is_empty() {
            continue;
        }
        items.push((start, end, snippet));
    }
    ...
    for (idx, (start, end, snippet)) in items.iter().enumerate() {
        if idx > 0 {
            let prev_end = items[idx - 1].1;
            let gap = source_slice(source, prev_end, *start).unwrap_or_default();
            if has_blank_line(gap) {
                out.push('\n');
            }
            out.push('\n');
        }
        out.push_str(&normalize_indentation(snippet, depth));
        if idx + 1 == items.len() {
            let tail = source_slice(source, *end, program_end(program)).unwrap_or_default();
            if tail.contains("\n\n") {
                out.push('\n');
            }
        }
...
fn render_program_body_with_comment_hooks(
    program: &EstreeNode,
    depth: usize,
    options: &crate::api::PrintOptions,
) -> String {
    ...
    for value in body.iter() {
        ...
        let statement = render(node).unwrap_or_default();
        if statement.trim().is_empty() {
            continue;
        }

        let mut lines = Vec::new();
        for comment in comment_texts(node, RawField::LeadingComments, options, true) {
            lines.push(format!("{}{}", tabs(depth), comment));
        }

        let trailing = comment_texts(node, RawField::TrailingComments, options, false);
        if trailing.len() == 1 && !statement.contains('\n') {
            let mut line = format!("{}{}", tabs(depth), statement);
            line.push(' ');
            line.push_str(&trailing[0]);
            lines.push(line);
        } else {
            lines.push(indent_block(&statement, depth));
...
fn render_attribute(source: &str, attribute: &Attribute) -> String {
    match attribute {
        ...
        Attribute::LetDirective(directive) => {
            ...
            let name = expression_identifier_name(&directive.expression);
            let shorthand = name
                .as_ref()
                .is_some_and(|value| value.as_ref() == directive.name.as_ref());
            let is_empty = name.as_ref().is_some_and(|value| value.is_empty());
...
fn comment_texts(
    ...
    let comments = if leading {
        options
            .get_leading_comments
            .as_ref()
            .map(|callback| callback.call(node))
    } else {
        options
    ...
    let comments = comments.unwrap_or_else(|| estree_comment_field(node, field));
    comments
        .iter()
        .filter_map(render_estree_comment)
        .collect::<Vec<_>>()
}

fn estree_comment_field(node: &EstreeNode, field: RawField) -> Box<[EstreeNode]> {
    let Some(EstreeValue::Array(values)) = estree_node_field(node, field) else {
        return Box::default();
    };
    ...
    values
        ...
        .filter_map(|value| match value {
            EstreeValue::Object(node) => Some(node.clone()),
            _ => None,
        })
        .collect::<Vec<_>>()
        .into_boxed_slice()
...
fn render_estree_comment(comment: &EstreeNode) -> Option<String> {
    let EstreeValue::String(kind) = estree_node_field(comment, RawField::Type)? else {
        return None;
    };
    let EstreeValue::String(value) = estree_node_field(comment, RawField::Value)? else {
        return None;
    };
...
fn program_end(program: &EstreeNode) -> usize {
    estree_value_to_usize(estree_node_field(program, RawField::End)).unwrap_or(0)
}

fn estree_value_to_usize(value: Option<&EstreeValue>) -> Option<usize> {
    match value? {
        EstreeValue::UInt(value) => usize::try_from(*value).ok(),
        EstreeValue::Int(value) if *value >= 0 => usize::try_from(*value).ok(),
        _ => None,
    }
}
