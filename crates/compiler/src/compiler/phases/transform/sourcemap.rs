...
fn add_sparse_pair_matches(input: SparseMatchInput<'_>, registry: SparseMatchRegistry<'_>) {
    ...
    for (original_offset, generated_offset) in pairs {
        register_segment(
            segments_by_generated,
            GeneratedSegment {
                generated_offset,
...
fn pair_sparse_offsets(
    ...
    if original_offsets.is_empty() || generated_offsets.is_empty() {
        return Vec::new();
    }
    ...
    if original_offsets.len() == 1 {
        return generated_offsets
            .iter()
            .copied()
            .map(|generated_offset| (original_offsets[0], generated_offset))
            .collect();
    }
    ...
    let original_span = source_len.saturating_sub(original_len);
    let generated_span = output_len.saturating_sub(generated_len);
    let mut next_generated_index = 0usize;
    let mut pairs = Vec::with_capacity(original_offsets.len().min(generated_offsets.len()));

    for &original_offset in original_offsets {
        if next_generated_index >= generated_offsets.len() {
            break;
        }

        let target_generated_offset = match std::num::NonZeroUsize::new(original_span) {
            Some(original_span) => {
                original_offset.saturating_mul(generated_span) / original_span.get()
            }
            None => generated_offsets[next_generated_index],
        };

        let mut best_index = next_generated_index;
        let mut best_score = sparse_context_score(
            source,
            original_offset,
            original_len,
            output,
            generated_offsets[best_index],
            generated_len,
        );
        let mut best_distance = generated_offsets[best_index].abs_diff(target_generated_offset);
        for (candidate_index, &candidate_offset) in generated_offsets
            .iter()
            .enumerate()
            .skip(next_generated_index + 1)
        {
            let candidate_score = sparse_context_score(
                source,
                original_offset,
                original_len,
                output,
            ...
            let candidate_distance = candidate_offset.abs_diff(target_generated_offset);
        ...
        pairs.push((original_offset, generated_offsets[best_index]));
        next_generated_index = best_index + 1;
    }

    pairs
}

fn sparse_context_score(
    source: &str,
    original_offset: usize,
    original_len: usize,
    output: &str,
    ...
    matching_suffix_len(
        source.get(..original_offset).unwrap_or_default(),
        output.get(..generated_offset).unwrap_or_default(),
        WINDOW,
    ) + matching_prefix_len(
        source
            .get(original_offset.saturating_add(original_len)..)
            .unwrap_or_default(),
        output
            .get(generated_offset.saturating_add(generated_len)..)
            .unwrap_or_default(),
        WINDOW,
    )
}

fn matching_suffix_len(left: &str, right: &str, limit: usize) -> usize {
    left.as_bytes()
        .iter()
        .rev()
...
fn matching_prefix_len(left: &str, right: &str, limit: usize) -> usize {
    left.as_bytes()
        .iter()
...
mod tests {
    ...
    #[test]
    fn sparse_pairing_prefers_generated_occurrences_near_relative_source_position() {
        let pairs = pair_sparse_offsets(
            &[36],
            "000000000000000000000000000000000000<h1>Hello</h1>",
            "<h1>Hello</h1>",
            &[120, 420],
            &format!(
                "{}<h1 class=\"x\">Hello</h1>{}",
                "x".repeat(120),
                "x".repeat(500 - 120 - "<h1 class=\"x\">Hello</h1>".len())
            ),
            "<h1 class=\"x\">Hello</h1>",
        );

        assert_eq!(pairs, vec![(36, 120), (36, 420)]);
    }

    #[test]
    fn sparse_pairing_prefers_generated_occurrences_with_matching_local_context() {
        let source = "<input bind:value={foo.bar.baz}>";
        let output = "$.bind_value(input, () => foo().bar.baz, ($$value) => foo(foo().bar.baz = $$value, true));";
        let original_offsets = find_all_occurrences(source, "bar.baz");
        let generated_offsets = find_all_occurrences(output, "bar.baz");
        ...
        assert_eq!(pairs, vec![(23, 32), (23, 64)]);
    }
}
