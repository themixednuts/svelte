...
use crate::ast::modern::{CssAtrule, CssBlock, CssBlockChild, CssNode, CssRule, Root};

use super::TextReplacement;
use super::usage::{
    BoundaryCandidates, CssAttributeFilter, CssAttributeMatchKind, CssElementUsage,
    CssUsageContext, EachBoundaryKind, build_css_usage_context,
};
...
pub(crate) fn collect_scoped_element_starts(source: &str, root: &Root) -> Box<[usize]> {
    let Some((_, _, content_start, content_end)) =
    ...
    let usage = build_css_usage_context(root, false);
    let mut starts = BTreeSet::new();
    collect_scoped_element_starts_in_nodes(
        &css_nodes,
        source,
        &usage,
        RewriteFrame::root(),
...
fn collect_scoped_element_starts_in_nodes(
    nodes: &[CssNode],
    source: &str,
    ...
    for node in nodes {
        match node {
            CssNode::Rule(rule) => {
                collect_scoped_element_starts_in_rule(rule, source, usage, frame, starts);
            }
            CssNode::Atrule(atrule) => {
                collect_scoped_element_starts_in_atrule(atrule, source, usage, frame, starts);
            }
        }
    }
}

fn collect_scoped_element_starts_in_rule(
    rule: &CssRule,
    source: &str,
    usage: &CssUsageContext,
    frame: RewriteFrame<'_>,
    ...
    if !frame.inside_keyframes && !frame.inside_global_block && !is_global_block_rule {
        for selector in split_selectors_top_level(prelude) {
            collect_scoped_element_starts_for_selector(&selector, usage, starts);
        }
    }

    collect_scoped_element_starts_in_block(
        &rule.block,
        source,
        usage,
        RewriteFrame {
...
fn collect_scoped_element_starts_in_atrule(
    atrule: &CssAtrule,
    source: &str,
    usage: &CssUsageContext,
    frame: RewriteFrame<'_>,
...
fn collect_scoped_element_starts_in_block(
    block: &CssBlock,
    source: &str,
    usage: &CssUsageContext,
    frame: RewriteFrame<'_>,
...
fn collect_scoped_element_starts_for_selector(
    selector: &str,
    usage: &CssUsageContext,
    starts: &mut BTreeSet<usize>,
) {
    ...
    let mut path = vec![None; parts.len()];
    for (index, _) in usage.elements.iter().enumerate() {
        if !selector_part_matches_element_for_scope(
            parts.last().map(|part| part.1.as_str()).unwrap_or_default(),
            index,
            usage,
        ) {
            continue;
        ...
        path[parts.len() - 1] = Some(index);
        collect_scoped_match_path_starts(&parts, parts.len() - 1, index, usage, &mut path, starts);
        path[parts.len() - 1] = None;
    }
}

fn collect_scoped_match_path_starts(
    parts: &[(String, String)],
    part_index: usize,
    element_index: usize,
    usage: &CssUsageContext,
    ...
    if part_index == 0 {
        for (index, maybe_element) in path.iter().enumerate() {
            if let Some(matched_element) = maybe_element
                && selector_part_is_local_for_scope(parts[index].1.as_str())
                && let Some(element) = usage.elements.get(*matched_element)
            {
                starts.insert(element.node_start);
            }
        }
        return;
    }
    ...
    let left_index = part_index - 1;
    for candidate in related_left_elements(element_index, combinator, usage) {
        if !selector_part_matches_element_for_scope(parts[left_index].1.as_str(), candidate, usage)
        {
            continue;
        }

        path[left_index] = Some(candidate);
        collect_scoped_match_path_starts(parts, left_index, candidate, usage, path, starts);
        path[left_index] = None;
    }
}

fn related_left_elements(
    index: usize,
    ...
    match relation {
        ...
        SimpleCombinator::Child => usage
            ...
            .into_iter()
            .collect(),
        SimpleCombinator::Adjacent => possible_previous_siblings(index, usage),
        SimpleCombinator::GeneralSibling => all_previous_siblings(index, usage),
        SimpleCombinator::Column => Vec::new(),
    }
}

fn selector_part_is_local_for_scope(part: &str) -> bool {
    let normalized = remove_bare_global_tokens(&remove_global_selector_regions(part));
    let trimmed = normalized.trim();
    !trimmed.is_empty() && trimmed != "&" && trimmed != "*"
}

fn selector_part_matches_element_for_scope(
    part: &str,
    element_index: usize,
    usage: &CssUsageContext,
) -> bool {
    ...
    let Some(element) = usage.elements.get(element_index) else {
        return false;
    };

    let tags = extract_selector_tag_names(trimmed);
    if !tags.is_empty() && !tags.iter().any(|tag| tag == element.tag.as_ref()) {
        return false;
    }

    let classes = extract_selector_class_names(trimmed);
    if !classes.is_empty() {
    ...
    let ids = extract_selector_id_names(trimmed);
    if !ids.is_empty() {
        let id_value = element.attributes.get("id").map(Arc::as_ref).unwrap_or("");
        if !ids.iter().all(|id| id_value == id) {
            return false;
        }
    }

    let attribute_filters = extract_selector_attribute_filters(trimmed);
    attribute_filters
        .iter()
