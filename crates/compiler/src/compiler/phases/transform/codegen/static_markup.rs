...
pub(crate) fn compile_static_markup_js(ctx: ComponentCodegenContext<'_>) -> Option<String> {
    let component_name = component_name_from_filename(ctx.filename);
    let mount_binding = static_mount_binding_name(ctx.root);

    if let Some(output) = compile_imports_only_component_js(
        ctx.source,
        ctx.target,
        ctx.root,
        ctx.hmr,
    ...
    let markup = if matches!(ctx.fragments, FragmentStrategy::Tree)
        || !ctx.scoped_element_starts.is_empty()
    {
        serialize_static_markup_fragment(
            &ctx.root.fragment,
            ctx.css_hash,
            ctx.scoped_element_starts,
        )?
    } else {
        extract_static_markup_template_from_root(ctx.source, ctx.root)?
    };
    ...
    match ctx.target {
        ...
        GenerateTarget::Client => {
            if matches!(ctx.fragments, FragmentStrategy::Tree) {
                return compile_static_tree_client_js(ctx, &component_name);
            }
        ...
        GenerateTarget::Server => {
            if let Some(option_text) = extract_static_option_inner_text(ctx.root) {
...
fn compile_static_tree_client_js(
    ctx: ComponentCodegenContext<'_>,
    component_name: &str,
) -> Option<String> {
    let tree_literal =
        serialize_tree_fragment(&ctx.root.fragment, ctx.css_hash, ctx.scoped_element_starts)?;
    let fragment_count = count_normalized_fragment_items(&ctx.root.fragment);
...
fn serialize_static_markup_fragment(
    fragment: &Fragment,
    css_hash: Option<&str>,
    ...
    for item in normalized_fragment_items(fragment).into_iter() {
        match item {
            ...
            NormalizedFragmentItem::Node(node) => {
                serialize_html_node(node, &mut output, css_hash, scoped_element_starts)?;
            }
        }
    }
    ...
    match node {
        ...
        Node::RegularElement(element) => {
            ...
            serialize_html_attributes(
                &element.attributes,
                output,
                scoped_hash_for_element(element.start, css_hash, scoped_element_starts),
            )?;
            ...
            for item in normalized_fragment_items(&element.fragment).into_iter() {
                match item {
                    NormalizedFragmentItem::Space => output.push(' '),
                    NormalizedFragmentItem::Node(node) => {
                        serialize_html_node(node, output, css_hash, scoped_element_starts)?;
                    }
                }
            }
            output.push_str("</");
...
fn serialize_html_attributes(
    ...
    output: &mut String,
    scoped_hash: Option<&str>,
) -> Option<()> {
    let mut rendered_class = false;
    for attribute in attributes {
        rendered_class |= serialize_html_attribute(attribute, output, scoped_hash)?;
    }
...
fn scoped_hash_for_element<'a>(
    start: usize,
    css_hash: Option<&'a str>,
    scoped_element_starts: &[usize],
) -> Option<&'a str> {
    css_hash.filter(|_| scoped_element_starts.binary_search(&start).is_ok())
}

fn serialize_html_attribute(
    attribute: &Attribute,
    output: &mut String,
    scoped_hash: Option<&str>,
) -> Option<bool> {
...
fn serialize_tree_fragment(
    fragment: &Fragment,
    css_hash: Option<&str>,
    scoped_element_starts: &[usize],
) -> Option<String> {
    let values = build_tree_values(fragment, css_hash, scoped_element_starts)?;
    Some(format_tree_array(&values, 1))
}

fn build_tree_values(
    fragment: &Fragment,
    css_hash: Option<&str>,
    scoped_element_starts: &[usize],
) -> Option<Vec<TreeValue>> {
    ...
    for item in normalized_fragment_items(fragment).into_iter() {
        match item {
            ...
            NormalizedFragmentItem::Node(node) => {
                values.push(build_tree_value(node, css_hash, scoped_element_starts)?)
            }
        }
    }
...
fn build_tree_value(
    ...
    match node {
        ...
        Node::RegularElement(element) => {
            let attributes = serialize_tree_attributes(
                element,
                scoped_hash_for_element(element.start, css_hash, scoped_element_starts),
            )?;
...
fn serialize_tree_attributes(
    element: &RegularElement,
    scoped_hash: Option<&str>,
) -> Option<String> {
    let attributes = element.attributes.as_ref();
    if attributes.is_empty() {
        return scoped_hash.map_or_else(
            || Some(String::from("null")),
            |hash| Some(format!("{{ class: {} }}", js_single_quoted_string(hash))),
        );
    }
        ...
        let value = match &attribute.value {
            ...
            AttributeValueList::Values(values) => {
                ...
                if attribute.name.as_ref() == "class"
                    && let Some(hash) = scoped_hash
                {
                    if !value_text.is_empty() {
                        value_text.push(' ');
                    }
                    value_text.push_str(hash);
                    rendered_class = true;
                }
    ...
    if let Some(hash) = scoped_hash
        && !rendered_class
    {
        entries.push(format!("class: {}", js_single_quoted_string(hash)));
    }
...
pub(super) fn escape_js_template_literal(value: &str) -> String {
    value
        .replace('\\', "\\\\")
        .replace('`', "\\`")
        .replace("${", "\\${")
...
pub(super) fn component_name_from_filename(filename: Option<&Utf8Path>) -> String {
