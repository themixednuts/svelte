...
use crate::compiler::phases::parse::{ParsedComponent, ParsedModuleProgram};
...
pub(crate) fn analyze_component<'a>(
    ...
    let scoped_element_starts =
        crate::compiler::phases::transform::css::collect_scoped_element_starts(
            parsed.as_ref(),
            parsed.root(),
        );
...
pub(crate) fn validate_module(parsed: &ParsedModuleProgram<'_>) -> Result<(), CompileError> {
    if let Some(error) = validation::validate_module_source(parsed) {
        return Err(error.with_source_text(parsed.source_text()));
    }
    Ok(())
}
...
pub(crate) fn collect_module_warnings(
    ...
    warnings::collect_module_warnings(parsed, options).into_boxed_slice()
}
