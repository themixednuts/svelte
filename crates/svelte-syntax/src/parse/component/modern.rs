...
pub(crate) fn parse_pattern_with_oxc(
    ...
    let base_column = column.saturating_sub(1);
    let expression =
        crate::parse::parse_modern_expression_with_oxc(&wrapped, global_start, line, base_column)?;
...
pub fn normalize_pattern_template_elements(node: &mut EstreeNode) {
    if matches!(
...
fn set_expression_character(source: &str, expression: &mut Expression) {
    ...
    let start_character = source_location_at_offset(source, start).character;
    let end_character = source_location_at_offset(source, end).character;
    if let Some(EstreeValue::Object(start_loc)) = estree_node_field_mut(loc, RawField::Start) {
        start_loc.fields.insert(
            "character".to_string(),
            EstreeValue::UInt(start_character as u64),
        );
    }
    if let Some(EstreeValue::Object(end_loc)) = estree_node_field_mut(loc, RawField::End) {
        end_loc.fields.insert(
            "character".to_string(),
            EstreeValue::UInt(end_character as u64),
        );
    }
}
...
pub fn parse_leading_comment_nodes(source: &str, global_start: usize) -> Vec<EstreeNode> {
    let bytes = source.as_bytes();
    let mut i = 0usize;
    let mut comments = Vec::new();
...
pub fn parse_all_comment_nodes(source: &str, global_start: usize) -> Vec<EstreeNode> {
    let bytes = source.as_bytes();
    let mut i = 0usize;
    let mut out = Vec::new();
...
pub fn attach_estree_comments_to_tree(
    root: &mut EstreeNode,
    source_fragment: &str,
    global_start: usize,
    comments: &[EstreeNode],
...
pub fn estree_value_to_usize(value: Option<&EstreeValue>) -> Option<usize> {
    match value {
...
pub fn estree_node_type(node: &EstreeNode) -> Option<&str> {
    estree_node_field_str(node, RawField::Type)
}

pub fn estree_node_field(node: &EstreeNode, key: RawField) -> Option<&EstreeValue> {
    node.fields.get(key.as_str())
}

pub fn estree_node_field_mut(node: &mut EstreeNode, key: RawField) -> Option<&mut EstreeValue> {
    node.fields.get_mut(key.as_str())
}

pub fn estree_node_has_field(node: &EstreeNode, key: RawField) -> bool {
    node.fields.contains_key(key.as_str())
}

pub fn estree_node_field_object(node: &EstreeNode, key: RawField) -> Option<&EstreeNode> {
...
pub fn estree_node_field_array(node: &EstreeNode, key: RawField) -> Option<&[EstreeValue]> {
    match estree_node_field(node, key) {
...
pub fn estree_node_field_str(node: &EstreeNode, key: RawField) -> Option<&str> {
    match estree_node_field(node, key) {
...
pub fn walk_estree_node(node: &EstreeNode, visitor: &mut impl FnMut(&EstreeNode)) {
    visitor(node);
...
pub fn walk_raw_value(value: &EstreeValue, visitor: &mut impl FnMut(&EstreeNode)) {
    match value {
...
pub fn position_raw_node(line: usize, column: usize) -> EstreeNode {
    let mut fields = BTreeMap::new();
