...
fn legacy_node_from_modern_loose(node: modern::Node) -> Option<legacy::Node> {
    match node {
        ...
        modern::Node::Comment(comment) => Some(legacy::Node::Comment(legacy::Comment {
            ...
            ignores: crate::parse::parse_svelte_ignores(&comment.data),
            data: comment.data,
        })),
...
fn legacy_expression_from_modern_with_source(
    ...
    let oxc_expression = expression.oxc_expression()?;
    legacy_expression_from_oxc_expression(source, oxc_expression, &expression, include_character)
}

fn legacy_expression_from_binding_pattern(
    source: &str,
...
fn legacy_expression_from_oxc_expression(
    source: &str,
    ...
    Some(match expression {
        ...
        OxcExpression::NullLiteral(_) => legacy::Expression::Literal(legacy::LiteralExpression {
            ...
            loc,
            value: legacy::LiteralValue::Null,
            raw: Arc::from("null"),
        }),
        ...
        OxcExpression::CallExpression(call) => {
            let callee = legacy_expression_from_oxc_expression(
                source,
                &call.callee,
                modern_expression,
                include_character,
            ...
            let arguments = call
                ...
                .filter_map(|argument| match argument {
                    OxcArgument::SpreadElement(_) => None,
                    _ => legacy_expression_from_oxc_expression(
                        source,
        ...
        OxcExpression::BinaryExpression(binary) => {
            let left = legacy_expression_from_oxc_expression(
                source,
                &binary.left,
                modern_expression,
                include_character,
            )?;
            let right = legacy_expression_from_oxc_expression(
                source,
                &binary.right,
                modern_expression,
                include_character,
            ...
            legacy::Expression::BinaryExpression(legacy::BinaryExpressionNode {
                ...
                loc,
                left: Box::new(left),
                operator: Arc::from(binary.operator.as_str()),
                right: Box::new(right),
            })
        }
...
fn legacy_expression_loc(
    ...
    include_character: bool,
) -> Option<legacy::ExpressionLoc> {
    if source.is_empty() {
        return None;
    }
    let (start_line, start_column) = line_column_at_offset(source, start);
    let (_end_line, end_column) = line_column_at_offset(source, end);
    Some(legacy_loc(
        source,
        start_line,
        start_column,
...
fn modern_identifier_expression_with_loc(
    ...
    let _ = (name, loc);
    modern::Expression::empty(start, end)
}
...
fn parse_legacy_element(source: &str, node: Node<'_>) -> legacy::Element {
    ...
    if matches!(classify_element_name(name.as_ref()), ElementKind::Textarea) {
        let content_start = start_tag
            .map(|tag| tag.end_byte())
            .unwrap_or(node.start_byte());
        let close_start = crate::parse::find_valid_legacy_closing_tag_start(
            source,
            content_start,
            node.end_byte(),
            "textarea",
        ...
        for attr_child in child.named_children(&mut attr_cursor) {
            match attr_child.kind() {
                ...
                "expression" => {
                    ...
                    let tag = if loose {
                        Some(parse_modern_expression_tag_loose(source, attr_child))
                    } else {
        ...
        "expression" => {
            if loose {
                value_parts.push(modern::AttributeValue::ExpressionTag(
                    parse_modern_expression_tag_loose(source, node),
                ));
...
fn parse_legacy_comment(source: &str, node: Node<'_>) -> legacy::Comment {
    ...
    let ignores = crate::parse::parse_svelte_ignores(data_text);
...
fn parse_legacy_snippet_block(
    ...
    let expression = legacy_expression_from_modern_with_source(source, expression, true)?;
    ...
    let parameters = super::modern::parse_snippet_params(source, params_node)
        ...
        .filter_map(|expression| {
            legacy_expression_from_modern_with_source(source, expression, false)
        })
        .collect::<Vec<_>>();
...
fn parse_legacy_expression_from_text(
    ...
    if let Some(mut modern_expression) =
        crate::parse::parse_modern_expression_with_oxc(trimmed, start, line, start_col)
    {
...
fn collect_legacy_document_comments(
    ...
    if let Some(script) = instance {
        collect_comments_from_script_content_range(source, script, &mut comments, &mut seen);
    }
