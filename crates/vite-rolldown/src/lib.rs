...
pub fn classify_request_id(id: &str) -> RequestKind {
    ...
    if path_lower.ends_with(".svelte.js") || path_lower.ends_with(".svelte.ts") {
...
pub fn request_kind_for_request(request: &TransformRequest) -> RequestKind {
    request
        .request_kind
        .unwrap_or_else(|| classify_request_id(&request.id))
}
...
impl SvelteBundlerBridge for RustCompilerBridge {
    ...
    fn transform(&self, request: TransformRequest) -> Result<TransformResult, Self::Error> {
        let kind = request_kind_for_request(&request);
        ...
        transform_with_options(&request, options)
    }
}
...
pub fn compile_options_for_request(
    request: &TransformRequest,
) -> Result<svelte_compiler::CompileOptions, svelte_compiler::CompileError> {
    let mut options: svelte_compiler::CompileOptions = request
        .compiler_options_json
        .as_deref()
        .map(serde_json::from_str)
        .transpose()
        .map_err(|error| {
            svelte_compiler::CompileError::internal(format!(
                "invalid bundler compiler options json: {error}"
            ))
        })?
        .unwrap_or_default();
...
pub fn transform_with_options(
    request: &TransformRequest,
    options: svelte_compiler::CompileOptions,
) -> Result<TransformResult, svelte_compiler::CompileError> {
    let kind = request_kind_for_request(request);
    match kind {
        RequestKind::SvelteComponent => Ok(transform_result_from_compile_result(
            svelte_compiler::compile(&request.code, options)?,
        )),
        RequestKind::SvelteModule => Ok(transform_result_from_compile_result(
            svelte_compiler::compile_module(&request.code, options)?,
        )),
        RequestKind::VirtualCss => Ok(TransformResult {
        ...
        RequestKind::Unknown => Err(svelte_compiler::CompileError::unimplemented(
            "bundler transform for unknown request kind",
        )),
    }
}

pub fn transform_result_from_compile_result(
    result: svelte_compiler::CompileResult,
) -> TransformResult {
    let map_json = result
        .js
        .map
    ...
    TransformResult {
        ...
        css: result
            .css
            .as_ref()
            .map(|artifact| artifact.code.to_string()),
        css_map_json: result
            .css
            .as_ref()
            .and_then(|artifact| artifact.map.as_ref())
            .and_then(|map| serde_json::to_string(map).ok()),
        css_has_global: result.css.as_ref().and_then(|artifact| artifact.has_global),
        warnings_json: serde_json::to_string(&result.warnings).ok(),
    }
}
...
mod tests {
    ...
    #[test]
    fn classifies_typescript_svelte_module_request() {
        assert_eq!(
            classify_request_id("/src/module.svelte.ts"),
            RequestKind::SvelteModule
        );
    }

    #[test]
    fn compile_options_json_is_applied_before_request_overrides() {
        let options = compile_options_for_request(&TransformRequest {
            ...
            request_kind: None,
            compiler_options_json: Some("{\"dev\":true,\"preserve_whitespace\":true}".to_string()),
        })
        .expect("compiler options should deserialize");

        assert!(options.dev);
        assert!(options.preserve_whitespace);
        assert_eq!(
            options.filename.as_deref().map(|path| path.as_str()),
            Some("/src/App.svelte"),
        );
        assert_eq!(options.generate, svelte_compiler::GenerateTarget::Client);
    }

    #[test]
    fn compile_options_json_accepts_numeric_component_api() {
        let options = compile_options_for_request(&TransformRequest {
            id: "/src/App.svelte".to_string(),
            code: "<h1>Hello</h1>".to_string(),
            ssr: false,
            ...
            target: BundlerTarget::Vite,
            request_kind: None,
            compiler_options_json: Some("{\"compatibility\":{\"componentApi\":4}}".to_string()),
        })
        .expect("compiler options should deserialize");
        ...
        assert_eq!(
            options
                .compatibility
                .as_ref()
                .and_then(|compatibility| compatibility.component_api),
            Some(svelte_compiler::CompatibilityComponentApi::V4)
        );
    }
    ...
    fn bridge_compiles_module_request() {
        ...
        assert!(result.code.contains("generated by Svelte v"));
    }
    ...
    fn bridge_passthroughs_virtual_css() {
        ...
        assert!(result.css_map_json.is_none());
    }
    ...
    #[test]
    fn explicit_request_kind_overrides_id_classification() {
        let bridge = RustCompilerBridge;
        let result = bridge
            .transform(TransformRequest {
                id: "/src/component.svx".to_string(),
                code: "<h1>Hello</h1>".to_string(),
                ssr: false,
                hmr: false,
                target: BundlerTarget::Vite,
                request_kind: Some(RequestKind::SvelteComponent),
                compiler_options_json: None,
            })
            .expect("explicit request kind should allow custom component extensions");
