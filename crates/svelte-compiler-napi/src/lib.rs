...
use svelte_compiler::ast::modern::EstreeNode;
...
use svelte_compiler::{
    ...
    PrintCommentGetterCallback, PrintOptions,
};
...
fn build_print_options_with_callbacks(
    ...
    let mut options = deserialize_or_default::<PrintOptions>(options_json)?;
    let callback_error = Arc::new(Mutex::new(None));

    if let Some(callback) = leading_comments_callback {
        let callback_error_ref = Arc::clone(&callback_error);
        options.get_leading_comments = Some(PrintCommentGetterCallback::new(move |node| {
            invoke_comment_callback(&env, &callback, node, &callback_error_ref)
        }));
    }

    if let Some(callback) = trailing_comments_callback {
        let callback_error_ref = Arc::clone(&callback_error);
        options.get_trailing_comments = Some(PrintCommentGetterCallback::new(move |node| {
            invoke_comment_callback(&env, &callback, node, &callback_error_ref)
        }));
    }
...
fn invoke_comment_callback(
    env: &Env,
    callback: &FunctionRef<(String,), String>,
    node: &EstreeNode,
    callback_error: &Arc<Mutex<Option<Error>>>,
) -> Box<[EstreeNode]> {
    let payload = match serde_json::to_string(node) {
        ...
        Err(error) => {
            store_callback_error(
                callback_error,
                Error::from_reason(format!("failed to serialize print callback node: {error}")),
            );
            return Box::new([]);
        }
    };

    let value = match call_js_function(env, callback, payload) {
        Ok(value) => value,
        Err(error) => {
    ...
    match serde_json::from_str::<Vec<EstreeNode>>(&value) {
        Ok(nodes) => nodes.into_boxed_slice(),
        Err(error) => {
            store_callback_error(
                callback_error,
                Error::from_reason(format!(
                    "print comment callback must return an array of ESTree comment nodes: {error}"
                )),
            );
            Box::new([])
        }
